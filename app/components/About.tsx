...
const About = ({setIsDisplayed, isDisplayed}: {setIsDisplayed: (val: boolean) => void, isDisplayed: boolean}) => {
  const bgProps = {
  ...
  return (
    <section id="about" className='px-6 lg:px-20 py-24 w-full flex lg:flex-row flex-col gap-[5%] items-stretch justify-between'>
      ...
      <div className='flex-1 flex flex-col lg:gap-8 gap-6 pt-10 lg:pt-0'>
        ...
        <h2 className='lg:text-5xl text-3xl font-bold capitalize pr-10'>When you think home, Think <span className='text-[#0098db]'>Rampart.</span></h2>
        <p>
        ...
        <div className='flex md:flex-row flex-col gap-6 items-center'>
          <Link href={"#contact-us"}>
            <Button onClick={() => setIsDisplayed(!isDisplayed)} className='bg-transparent max-sm:w-full h-12 border hover:bg-gray-200 hover:text-[#45355f] hover:border-gray-500 border-[#45355f] rounded-lg text-[#45355f]'>Make an appointment</Button>
          </Link>
